...
CURRENT_VERSION=$(/usr/libexec/PlistBuddy LNTouchVisualizer/LNTouchVisualizer/Info.plist -c "Print CFBundleShortVersionString")
...
/usr/libexec/PlistBuddy LNTouchVisualizer/LNTouchVisualizer/Info.plist -c "Set CFBundleShortVersionString $NEXT_VERSION" -c "Set CFBundleVersion 1"
...
gh release create --repo LeoNatan/LNTouchVisualizer "$NEXT_VERSION" --title "v$NEXT_VERSION" --notes-file "${RELEASE_NOTES_FILE}"
